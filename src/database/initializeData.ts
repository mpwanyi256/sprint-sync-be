...
const populateAPIKeys = async () => {
    try {
        ...
        const existingKey = await mongoose.connection.collection('api_keys').findOne({});
        
        if (!existingKey) {
            const result = await mongoose.connection.collection('api_keys').insertOne({
    ...
    try {
        switch (collectionName) {
            ...
            case 'users':
                ...
                break;
            default:
                Logger.warn(`Collection initialization not implemented for: ${collectionName}`);
        }
    } catch (error) {
