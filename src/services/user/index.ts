...
export class UserService {
  ...
  async createUserWithKeystore(
    ...
    const keystore = await this.keystoreRepo.create(
      user,
      accessKey,
      refreshKey
    );
    ...
    Logger.info(
      `User successfully created and keystore assigned: ${userData.email}`
    );
  ...
  async getAllUsersWithPagination(
    ...
    Logger.debug(
      `Fetching users with pagination. Page: ${pagination.page}, Limit: ${pagination.limit}, Filters:`,
      JSON.stringify(filters)
    );

    const result = await this.userRepo.getAllUsersWithPagination(
      filters,
      pagination
    );
...
export const createUser = async (
  ...
  return await userService.createUserWithKeystore(
